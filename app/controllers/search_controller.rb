class SearchController < ApplicationController
  def index
    ...
    @articles= @results.find_all { |result| result.class == Content && !result.article.nil? && !result.draft }
    @discussions = @results.find_all { |result| result.class == Discussion && result.is_visible_to(current_user) }
  ...
  def execute_search(search_string)
    search = Ultrasphinx::Search.new :query => search_string, :per_page => 100, :filters => {'community_id' => current_community.id}
    search.excerpt
